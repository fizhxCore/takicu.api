...
export default function DocsPage() {
  ...
  const [globalApiKey, setGlobalApiKey] = useState(settings.apiKey)
  const [selectedTag, setSelectedTag] = useState('ALL')
  const [copiedState, setCopiedState] = useState({})

  useEffect(() => {
    const savedKey = localStorage.getItem('kyzz_global_apikey')
    if (savedKey) {
  ...
  const handleGlobalApiKeyChange = (val) => {
    setGlobalApiKey(val)
    localStorage.setItem('kyzz_global_apikey', val)
  }
  ...
  return (
    <div className="page-wrapper docs-wrapper">
      ...
      <div className="container" style={{ marginTop: '1rem' }}>
        {/* Info Card Header */}
        <div className="info-card" style={{ padding: '1.5rem', marginBottom: '1.25rem', background: 'linear-gradient(135deg, var(--surface) 0%, var(--bg) 100%)', border: '1px solid var(--border-color)', borderRadius: '12px', boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              <div style={{ width: '42px', height: '42px', borderRadius: '10px', background: 'rgba(242, 169, 59, 0.12)', border: '1px solid rgba(242, 169, 59, 0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)', flexShrink: 0 }}>
              ...
              <div>
                ...
                <p style={{ color: 'var(--text-secondary)', margin: '0.2rem 0 0', fontSize: '0.875rem', fontWeight: 500 }}>
                  High-performance, developer-first REST API platform maintained by <strong style={{ color: 'var(--text-primary)' }}>{settings.creator}</strong>.
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span className="badge badge-online" style={{ fontSize: '0.75rem', padding: '4px 10px', fontWeight: 700 }}>OAS 3.0.0</span>
              <span className="badge" style={{ fontSize: '0.75rem', padding: '4px 10px', background: 'rgba(242, 169, 59, 0.12)', color: 'var(--primary)', borderColor: 'rgba(242, 169, 59, 0.35)', fontWeight: 700 }}>v{settings.version}</span>
            </div>
          </div>

          {/* Video Loop Muted Showcase — TODO: video ini masih rekaman UI lama (Kyzz APIs v2), rekam ulang setelah branding/tema baru siap */}
          <div style={{ marginTop: '1rem', width: '100%', display: 'flex', justifyContent: 'center', background: 'var(--bg)', borderRadius: '10px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
            <video
              src="/docs.mp4"
              autoPlay
              loop
        ...
        <div style={{ margin: '0.75rem 0', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--surface)', padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
            ...
            <input
              ...
              placeholder={`e.g. ${settings.apiKey}`}
              style={{
                flex: 1,
                padding: '4px 8px',
                fontSize: '0.8rem',
            ...
            <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '4px' }}>
              <button
                ...
                style={{
                  padding: '4px 10px',
                  borderRadius: '20px',
                  border: selectedTag === 'ALL' ? '1px solid var(--primary)' : '1px solid var(--border-color)',
                  background: selectedTag === 'ALL' ? 'rgba(242, 169, 59, 0.12)' : 'var(--surface)',
                  color: selectedTag === 'ALL' ? 'var(--primary)' : 'var(--text-secondary)',
                  fontSize: '0.75rem',
                  fontWeight: 700,
                  cursor: 'pointer',
                  whiteSpace: 'nowrap'
              ...
              {Object.keys(groupedEndpoints).map(catName => (
                <button
                  ...
                  style={{
                    padding: '4px 10px',
                    borderRadius: '20px',
                    border: selectedTag === catName ? '1px solid var(--primary)' : '1px solid var(--border-color)',
                    background: selectedTag === catName ? 'rgba(242, 169, 59, 0.12)' : 'var(--surface)',
                    color: selectedTag === catName ? 'var(--primary)' : 'var(--text-secondary)',
                    fontSize: '0.75rem',
                    fontWeight: 700,
                    cursor: 'pointer',
                    whiteSpace: 'nowrap'
        ...
        {loading && (
          <div className="info-card" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '3rem 1rem' }}>
            Loading API specifications...
          </div>
        )}

        {error && (
          <div className="info-card" style={{ color: 'var(--danger)', background: 'rgba(240, 85, 91, 0.12)', borderColor: 'rgba(240, 85, 91, 0.35)', padding: '1.5rem' }}>
                ...
                >
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span className="badge" style={{ fontSize: '0.7rem', padding: '1px 6px', background: 'var(--border-color)', color: 'var(--text-secondary)' }}>
                            ...
                            {isExpanded && (
                              <div className="op-content">
                                ...
                                {response && (() => {
                                  ...
                                  return (
                                    <div className="response-box" style={{ marginTop: '1rem', position: 'relative', background: 'var(--surface)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem', boxShadow: 'var(--shadow-sm)' }}>
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px', flexWrap: 'wrap', gap: '8px' }}>
                                        <div className="res-header" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary)' }}>
                                          Response Status: 
                                          <span style={{
                                            padding: '2px 8px',
                                            borderRadius: '4px',
                                            background: response.status >= 200 && response.status < 300 ? 'rgba(62, 207, 142, 0.12)' : 'rgba(240, 85, 91, 0.12)',
                                            color: response.status >= 200 && response.status < 300 ? 'var(--success)' : 'var(--danger)',
                                            border: response.status >= 200 && response.status < 300 ? '1px solid rgba(62, 207, 142, 0.35)' : '1px solid rgba(240, 85, 91, 0.35)',
                                            fontWeight: 700
                                          }}>
                                            {response.status}
                                          </span>
                                        ...
                                        {response.latency && (
                                          <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)', background: 'var(--surface-hover)', padding: '2px 8px', borderRadius: '4px', border: '1px solid var(--border-color)', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                                      ...
                                      <div style={{ marginBottom: '12px', background: 'var(--bg)', padding: '10px 12px', borderRadius: '6px', fontSize: '0.8rem', border: '1px solid var(--border-color)' }}>
                                        {/* Request URL */}
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                                          <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', fontWeight: 600 }}>Request URL:</span>
                                          <button
                                            ...
                                            style={{
                                              ...
                                              fontWeight: 600,
                                              cursor: 'pointer',
                                              color: isUrlCopied ? 'var(--success)' : 'var(--text-secondary)'
                                            }}
                                          >
                                            {isUrlCopied ? '✓ Copied URL' : 'Copy URL'}
                                          </button>
                                        </div>
                                        <code style={{ background: 'var(--surface)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', padding: '6px 8px', borderRadius: '4px', display: 'block', wordBreak: 'break-all', fontFamily: 'JetBrains Mono, monospace', marginBottom: '10px', fontSize: '0.775rem' }}>
                                          <a href={response.url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)', textDecoration: 'none', fontWeight: 500 }}>
                                            {fullUrl}
                                          </a>
                                        </code>
                                        ...
                                        {/* cURL Command */}
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                                          <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', fontWeight: 600 }}>cURL Command:</span>
                                          <button
                                            ...
                                            style={{
                                              ...
                                              fontWeight: 600,
                                              cursor: 'pointer',
                                              color: isCurlCopied ? 'var(--success)' : 'var(--text-secondary)'
                                            }}
                                          >
                                            {isCurlCopied ? '✓ Copied cURL' : 'Copy cURL'}
                                          </button>
                                        </div>
                                        <code style={{ background: 'var(--surface)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', padding: '6px 8px', borderRadius: '4px', display: 'block', wordBreak: 'break-all', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.775rem' }}>
                                          {curlCmd}
                                        </code>
                                      </div>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '6px' }}>
                                          <button
                                            ...
                                            style={{
                                              ...
                                              fontWeight: 600,
                                              cursor: 'pointer',
                                              color: isJsonCopied ? 'var(--success)' : 'var(--text-primary)'
                                            }}
                                          >
