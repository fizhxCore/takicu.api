...
const faqs = [
  {
    question: `Apakah ${settings.name} gratis digunakan?`,
    answer: `Ya, seluruh REST API services di ${settings.name} dapat diakses dan digunakan secara gratis dengan menggunakan API Key publik resmi (\`${settings.apiKey}\`).`
  },
  {
    question: 'Bagaimana cara menggunakan API Key?',
    answer: `Anda cukup menambahkan parameter query \`?apikey=${settings.apiKey}\` pada setiap request URL endpoint (contoh: \`/api/stats?apikey=${settings.apiKey}\`) atau menyertakannya pada header HTTP \`x-api-key\`.`
  },
  {
  ...
  {
    question: 'Bagaimana cara mendapatkan bantuan atau laporan bug?',
    answer: 'Anda dapat bergabung dengan WhatsApp Support Channel resmi kami melalui link Support yang tersedia di menu navigasi atau footer.'
  }
]
      ...
      <div className="container" style={{ maxWidth: '800px' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
            Pertanyaan yang sering ditanyakan seputar penggunaan {settings.name}.
          </p>
        </div>
